...
Luego iteramos a lo largo de las primeras cuatro capas para obtener todas las probabilidades madre.
...
b. Qué porcentaje de los que encuentr an la presencia de daños por patógenos alta utiliza
Metil Tiofanato 70%.
...
* Definimos la probabilidad que se debe hallar como P(A)
A: Sistema el sistema de riego funcione adecuadamente con un rendimiento por encima del promedio.

$$ B_1: \text{ Probabilidad de que el asperjador 1 funcione adecuadamente }$$
$$ B_2: \text{ Probabilidad de que el asperjador 2 funcione adecuadamente }$$
y así sucesivamente,

$$P(A)> 0.99999 $$
$$P(A)= P(B_1\cap B_2\dots\cap B_k)= 0.95^k $$
* Como las probabilidades de funcionamiento adecuado de los asperjadores son iguales y son independientes entre si, solo hay que despejar k

$$ k\text{log} (0.95) = log (0.99999) $$
```{r}

log10(0.99999)/log10(0.95)
```

*k = 0.000195, no es posible llegar a esa probabilidad de 0.99999 con esa probabilidad de que el asperjador 1 funcione adecuadamente.


# Tercer punto
3. Un médico veterinario ha decidido recetar dos nuevos medicamentos a 100 animales
enfermos del corazón y lo hace en la forma siguiente: 35 reciben el medicamento A, 35
reciben el medicamento B y 30 ambos medicamentos. El fármaco A reduce la
probabilidad de ataque cardiaco en 25%, el B lo hace en un 30% y los dos medicamentos,
si se toman juntos, funcionan de manera independiente. Se escogió a los 100 animales de
forma que cada uno tuviera 70% de probabilidad de sufrir un ataque cardiaco. Si un
animal seleccionado aleatoriamente sufre un ataque cardiaco.¿ Cuál es la probabilidad de
...
PA1*PA4
```
* La probabilidad de que el primero y cuarto anuncios sean vistos, sin que el segundo y tercero sean notados es de 0.675
...
$$P(A_1\cup A_2\cup A_3\cup A_4)^C $$
```{r}
...
$$P(B_1\cap B_2\cap B_3)^C $$

```{r}
PB1+PB2+PB3-(PB1*PB2)-(PB1*PB3)-(PB2*PB3)+(PB1*PB2*PB3)-(PB1*PB2*PB3)
```
...
a. Cuál es la probabilidad de que el segundo insecto sea dañino para el cultivo si el primero
lo fue.
...
a. Cuál es la probabilidad de que un sitio seleccionado al azar tenga un desgarre y haya
sido identificado como tal mediante MRI?
b. Cuál es la probabilidad de que un sitio elegido al azar no tenga ningún desgarre y haya
sido identificado como un desgarre?
c. Cuál es la probabilidad de que un sitio seleccionado al azar tenga un desgarre y no haya
sido identificado por MRI?
d. Si hay un desgarre, cuál es la probabilidad de una MRI positiva?
e. Si hay un desgarre, cuál es la probabilidad de un falso negativo; es decir, una MRI
negativa?
...
susceptibles.
# Decimo punto
10. Suponga que le interesa seguir dos rasgos independientes de los guisantes; textura
(S=lisa, s=rugosa) y color de la semilla (Y=amarilla, y=verde), en una cruza de segunda
generación de padres heterocigotos. Recuerde que la letra mayúscula representa el rasgo
dominante. Complete la tabla con los pares de genes para ambos rasgos. Todos los pares
...
e. Si una descendencia tiene guisantes amarillos lisos, Cuál es la probabilidad de que lleve
un alelo s? Un alelo s y uno y?

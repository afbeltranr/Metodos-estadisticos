...
enfermos del corazón y lo hace en la forma siguiente: 35 reciben el medicamento A, 35 reciben el medicamento B y 30 ambos medicamentos. El fármaco A reduce la
...
F: Sufrir un ataque cardiaco
C: Recibe los dos medicamentos
Si recibe los dos medicamentos al tiempo, la probabilidad de sufrir un ataque cardiaco baja a 15%
$$P(A\cap C) $$
```{r}
PF<-0.15
PC<-0.3
PF*PC
```
* Si un animal seleccionado aleatoriamente sufre un ataque cardiaco, la probabilidad de que le haya administrado los dos medicamentos es de 0.045
...
```
%YS = 12/16 = 0.75
* Un 75% de la descendencia de esta cruza tendrá guisantes amarillos lisos
...
6/12
```
* Si una descendencia tiene guisantes amarillos lisos la probabilidad de que lleve un alelo s es de 0.5

Un alelo s y uno y?
```{r}
4/12
```
* Si una descendencia tiene guisantes amarillos lisos la probabilidad de que lleve un alelo s y uno y es de 0.333
